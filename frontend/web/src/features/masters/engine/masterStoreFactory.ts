...
/**
 * One HTTP-backed store per entity, keyed by config.key — same MasterStore interface
 * regardless of entity, so the generic list/form pages and hooks built on top of it don't
 * need per-entity code. Talks to /api/v1/masters/* via the shared MastersApi client.
 */
...
export function createMasterHttpStore(config: MasterEntityConfig): MasterStore {
