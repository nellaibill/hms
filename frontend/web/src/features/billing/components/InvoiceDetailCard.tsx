import { Printer, Receipt, TriangleAlert } from 'lucide-react';
...
export function InvoiceDetailCard({
  ...
  return (
    <>
      <Card>
        <CardHeader className="flex-row items-center gap-3 space-y-0">
          ...
          <div className="flex flex-1 flex-col gap-1">
            ...
            <CardDescription>
              Invoice {billing.invoiceNumber ?? billing.id} ·{' '}
            ...
            <div className="flex items-start gap-2 rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive">
              ...
              <div className="flex flex-col">
                <span className="font-medium">
                  Voided
                  {billing.voidedAt
                    ? ` on ${new Date(billing.voidedAt).toLocaleString('en-IN')}`
                    : ''}
                </span>
      ...
      <InvoicePrintTemplate billing={billing} />
    </>
  );
}
