...
/// <summary>Warehouse master CRUD (no hard delete — see docs/03_Masters_ERD's is_active note; deactivate via PUT instead).</summary>
[ApiController]
...
public class WarehousesController : ControllerBase
{
    ...
    public WarehousesController(IWarehouseService service, IValidator<CreateWarehouseRequest> createValidator, IValidator<UpdateWarehouseRequest> updateValidator)
    {
        _service = service;
    ...
    /// <summary>Lists warehouses with paging, search, and active-status filtering.</summary>
    [HttpGet]
    public async Task<IActionResult> GetPaged([FromQuery] WarehouseListQuery query, CancellationToken cancellationToken)
    {
        ...
        return Ok(new ApiResponse<IReadOnlyList<WarehouseResponse>> { Data = paged.Items, Meta = meta });
    }

    private static ApiResponse<WarehouseResponse> Envelope(WarehouseResponse? data) => new() { Data = data };
