...
/// <summary>Payment method master CRUD (no hard delete — see docs/03_Masters_ERD's is_active note; deactivate via PUT instead).</summary>
[ApiController]
[Route("api/v1/masters/payment-methods")]
public class PaymentMethodsController : ControllerBase
{
    ...
    public PaymentMethodsController(IPaymentMethodService service, IValidator<CreatePaymentMethodRequest> createValidator, IValidator<UpdatePaymentMethodRequest> updateValidator)
    {
        _service = service;
    ...
    public async Task<IActionResult> GetPaged([FromQuery] PaymentMethodListQuery query, CancellationToken cancellationToken)
    {
        ...
        return Ok(new ApiResponse<IReadOnlyList<PaymentMethodResponse>> { Data = paged.Items, Meta = meta });
    }

    private static ApiResponse<PaymentMethodResponse> Envelope(PaymentMethodResponse? data) => new() { Data = data };
