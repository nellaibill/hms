...
/// <summary>Customer master CRUD (no hard delete — see docs/03_Masters_ERD's is_active note; deactivate via PUT instead).</summary>
[ApiController]
...
public class CustomersController : ControllerBase
{
    ...
    public CustomersController(ICustomerService service, IValidator<CreateCustomerRequest> createValidator, IValidator<UpdateCustomerRequest> updateValidator)
    {
        _service = service;
    ...
    /// <summary>Lists customers with paging, search, and active-status filtering.</summary>
    [HttpGet]
    public async Task<IActionResult> GetPaged([FromQuery] CustomerListQuery query, CancellationToken cancellationToken)
    {
        ...
        return Ok(new ApiResponse<IReadOnlyList<CustomerResponse>> { Data = paged.Items, Meta = meta });
    }

    private static ApiResponse<CustomerResponse> Envelope(CustomerResponse? data) => new() { Data = data };
