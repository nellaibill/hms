...
/// <summary>Manufacturer master CRUD (no hard delete — see docs/03_Masters_ERD's is_active note; deactivate via PUT instead).</summary>
[ApiController]
[Route("api/v1/masters/manufacturers")]
public class ManufacturersController : ControllerBase
{
    private readonly IManufacturerService _service;
    private readonly IValidator<CreateManufacturerRequest> _createValidator;
    private readonly IValidator<UpdateManufacturerRequest> _updateValidator;

    public ManufacturersController(IManufacturerService service, IValidator<CreateManufacturerRequest> createValidator, IValidator<UpdateManufacturerRequest> updateValidator)
    {
        _service = service;
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateManufacturerRequest request, CancellationToken cancellationToken)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateManufacturerRequest request, CancellationToken cancellationToken)
    {
    ...
    /// <summary>Lists manufacturers with paging, search, and active-status filtering.</summary>
    [Authorize]
    [RequirePermission("identity-administration.view")]
    [HttpGet]
    public async Task<IActionResult> GetPaged([FromQuery] ManufacturerListQuery query, CancellationToken cancellationToken)
    {
        ...
        return Ok(new ApiResponse<IReadOnlyList<ManufacturerResponse>> { Data = paged.Items, Meta = meta });
    }

    private static ApiResponse<ManufacturerResponse> Envelope(ManufacturerResponse? data) => new() { Data = data };
