...
/// <summary>Product sub category master CRUD (no hard delete — see docs/03_Masters_ERD's is_active note; deactivate via PUT instead).</summary>
[ApiController]
[Route("api/v1/masters/product-sub-categories")]
public class ProductSubCategoriesController : ControllerBase
{
    ...
    public ProductSubCategoriesController(IProductSubCategoryService service, IValidator<CreateProductSubCategoryRequest> createValidator, IValidator<UpdateProductSubCategoryRequest> updateValidator)
    {
        _service = service;
    ...
    public async Task<IActionResult> GetPaged([FromQuery] ProductSubCategoryListQuery query, CancellationToken cancellationToken)
    {
        ...
        return Ok(new ApiResponse<IReadOnlyList<ProductSubCategoryResponse>> { Data = paged.Items, Meta = meta });
    }

    private static ApiResponse<ProductSubCategoryResponse> Envelope(ProductSubCategoryResponse? data) => new() { Data = data };
