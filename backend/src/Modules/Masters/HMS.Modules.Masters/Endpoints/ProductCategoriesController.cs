...
/// <summary>Product category master CRUD (no hard delete — see docs/03_Masters_ERD's is_active note; deactivate via PUT instead).</summary>
[ApiController]
[Route("api/v1/masters/product-categories")]
public class ProductCategoriesController : ControllerBase
{
    ...
    public ProductCategoriesController(IProductCategoryService service, IValidator<CreateProductCategoryRequest> createValidator, IValidator<UpdateProductCategoryRequest> updateValidator)
    {
        _service = service;
    ...
    /// <summary>Gets a single product category by id.</summary>
    [Authorize]
    ...
    /// <summary>Lists product categories with paging, search, and active-status filtering.</summary>
    [Authorize]
    [RequirePermission("identity-administration.view")]
    [HttpGet]
    public async Task<IActionResult> GetPaged([FromQuery] ProductCategoryListQuery query, CancellationToken cancellationToken)
    {
        ...
        return Ok(new ApiResponse<IReadOnlyList<ProductCategoryResponse>> { Data = paged.Items, Meta = meta });
    }

    private static ApiResponse<ProductCategoryResponse> Envelope(ProductCategoryResponse? data) => new() { Data = data };
