...
/// <summary>Supplier master CRUD (no hard delete — see docs/03_Masters_ERD's is_active note; deactivate via PUT instead).</summary>
[ApiController]
...
public class SuppliersController : ControllerBase
{
    ...
    public SuppliersController(ISupplierService service, IValidator<CreateSupplierRequest> createValidator, IValidator<UpdateSupplierRequest> updateValidator)
    {
        _service = service;
    ...
    /// <summary>Lists suppliers with paging, search, and active-status filtering.</summary>
    [HttpGet]
    public async Task<IActionResult> GetPaged([FromQuery] SupplierListQuery query, CancellationToken cancellationToken)
    {
        ...
        return Ok(new ApiResponse<IReadOnlyList<SupplierResponse>> { Data = paged.Items, Meta = meta });
    }

    private static ApiResponse<SupplierResponse> Envelope(SupplierResponse? data) => new() { Data = data };
