...
public class ProductAttributeValuesController : ControllerBase
{
    ...
    private readonly IValidator<CreateProductAttributeValueRequest> _createValidator;
    private readonly IValidator<UpdateProductAttributeValueRequest> _updateValidator;

    public ProductAttributeValuesController(IProductAttributeValueService service, IValidator<CreateProductAttributeValueRequest> createValidator, IValidator<UpdateProductAttributeValueRequest> updateValidator)
    {
        _service = service;
    ...
    public async Task<IActionResult> GetPaged(Guid productId, [FromQuery] ProductAttributeValueListQuery query, CancellationToken cancellationToken)
    {
        ...
        return Ok(new ApiResponse<IReadOnlyList<ProductAttributeValueResponse>> { Data = paged.Items, Meta = meta });
    }

    private static ApiResponse<ProductAttributeValueResponse> Envelope(ProductAttributeValueResponse? data) => new() { Data = data };
